          ...
          {GAME_CARDS.map((card, i) => {
            ...
            return (
              <div key={card.label} className="game-card" onClick={()=>openWhatsApp(`Hello, I want to play ${card.label} on MangoPlay!`)}
                ...
                <div style={{ position:"absolute", top:10, right:-22, background:"linear-gradient(135deg,#ff9900,#ff4444)", padding:"3px 28px", transform:"rotate(45deg)", fontSize:8, fontWeight:900, color:"#000", letterSpacing:1, zIndex:3 }}>{BADGES[i]}</div>
                <div style={{ position:"relative", zIndex:2 }}>
                  <div style={{ fontWeight:900, fontSize:15, color:"#fff", fontFamily:"'Oswald',sans-serif", letterSpacing:2, marginBottom:5, textShadow:`0 0 12px ${card.glow}` }}>{card.label}</div>
                  <span style={{ fontSize:10, color:card.glow, fontWeight:700, background:"rgba(0,0,0,0.35)", padding:"2px 8px", borderRadius:10, backdropFilter:"blur(4px)" }}>{MULTIPLIERS[i]}</span>
                </div>
                <span className="card-emoji" style={{ position:"absolute", bottom:8, right:10, zIndex:2, fontSize:32, filter:`drop-shadow(0 0 8px ${card.glow})`, animation:`float ${2.8+i*0.3}s ease-in-out infinite`, animationDelay:`${i*0.2}s`, transition:"filter 0.3s", opacity:0.92 }}>{card.emoji}</span>
              </div>
            );
          })}
        </div>
