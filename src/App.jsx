...
const GlobalStyles = () => (
  <style>{`
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .app-container{ width:100%; margin:0 auto; position:relative; min-height:100vh; background:#0d0d1a; }
    @media (min-width:1024px){ .app-container{ max-width:1200px; } }
...
const BANNERS = [
  { type:"mango_welcome", title:"WELCOME TO", brand:"MANGO", sub:"ONLINE BOOK", bonus:"GET 10% BONUS ON NEW ID", bg:"linear-gradient(135deg,#0a0025 0%,#1a0050 40%,#0a1a40 100%)", accent:"#f90", emoji:"🏏", particle:"⭐" },
  { title:"LIVE CASINO", brand:"EVOLUTION", sub:"GAMING", bonus:"🃏 UNLIMITED WINS AWAIT YOU", bg:"linear-gradient(135deg,#200000 0%,#4a0010 45%,#001800 100%)", accent:"#ff4444", emoji:"🎰", particle:"💎" },
  { title:"INSTANT WIN", brand:"COLOR", sub:"PREDICTION", bonus:"🎲 WIN UP TO 9X YOUR BET", bg:"linear-gradient(135deg,#001a40 0%,#003380 45%,#000d1a 100%)", accent:"#00ccff", emoji:"🎲", particle:"✨" },
  { title:"MINES GAME", brand:"INSTANT", sub:"PAYOUT", bonus:"💣 MAX MULTIPLIER 2500X", bg:"linear-gradient(135deg,#001400 0%,#003300 45%,#000a00 100%)", accent:"#00ff88", emoji:"💣", particle:"🔥" },
];

const CATEGORY_TABS = [
  {
    label:"Teen Patti", glow:"#cc44ff", bg:"linear-gradient(135deg,#2a0050,#6a00aa)",
    scene:(<svg viewBox="0 0 80 52" width="80" height="52" style={{position:"absolute",inset:0}}>
      <ellipse cx="40" cy="38" rx="36" ry="12" fill="#1a4a1a" opacity="0.6"/>
      <rect x="6" y="10" width="22" height="30" rx="3" fill="#fff" opacity="0.9"/><rect x="6" y="10" width="22" height="30" rx="3" fill="none" stroke="#ddd" strokeWidth="0.8"/>
      ...
      <rect x="29" y="6" width="22" height="30" rx="3" fill="#fff" opacity="0.95"/><rect x="29" y="6" width="22" height="30" rx="3" fill="none" stroke="#ddd" strokeWidth="0.8"/>
      ...
      <rect x="52" y="10" width="22" height="30" rx="3" fill="#fff"/><rect x="52" y="10" width="22" height="30" rx="3" fill="none" stroke="#ddd" strokeWidth="0.8"/>
  ...
  {
    label:"Cricket", glow:"#ffaa00", bg:"linear-gradient(135deg,#1a0a00,#3a1800)",
    scene:(<svg viewBox="0 0 80 52" width="80" height="52" style={{position:"absolute",inset:0}}>
  ...
  {
    label:"Football", glow:"#44dd44", bg:"linear-gradient(135deg,#001800,#004400)",
    scene:(<svg viewBox="0 0 80 52" width="80" height="52" style={{position:"absolute",inset:0}}>
      ...
      {[0,1,2,3].map(j=>(<line key={j} x1={10+j*4} y1="20" x2={10+j*4} y2="36" stroke="#fff" strokeWidth="0.5" opacity="0.25"/>))}
  ...
  {
    label:"Satta Matka", glow:"#ff8822", bg:"linear-gradient(135deg,#1a0800,#4a2000)",
    scene:(<svg viewBox="0 0 80 52" width="80" height="52" style={{position:"absolute",inset:0}}>
  ...
  {
    label:"Andar Bahar", glow:"#00ddff", bg:"linear-gradient(135deg,#001828,#003858)",
    scene:(<svg viewBox="0 0 80 52" width="80" height="52" style={{position:"absolute",inset:0}}>
      <ellipse cx="40" cy="46" rx="38" ry="8" fill="#0a3a1a" opacity="0.7"/>
      {[0,1,2].map(j=>(<rect key={j} x={5+j*1.5} y={8+j*1.5} width="20" height="28" rx="2.5" fill={j===2?"#fff":"#e8e8e8"} stroke="#ccc" strokeWidth="0.6"/>))}
      ...
      {[0,1,2].map(j=>(<rect key={j} x={55-j*1.5} y={8+j*1.5} width="20" height="28" rx="2.5" fill={j===2?"#fff":"#e8e8e8"} stroke="#ccc" strokeWidth="0.6"/>))}
  ...
  {
    label:"Lucky 7", glow:"#ffdd00", bg:"linear-gradient(135deg,#1a1000,#3a2800)",
    scene:(<svg viewBox="0 0 80 52" width="80" height="52" style={{position:"absolute",inset:0}}>
      ...
      <circle cx="10" cy="6" r="3" fill="#ffcc00"/><circle cx="70" cy="6" r="3" fill="#ffcc00"/>
      <circle cx="10" cy="46" r="3" fill="#ffcc00"/><circle cx="70" cy="46" r="3" fill="#ffcc00"/>
    ...
    <div style={{ display:"flex", overflowX:"auto", background:"#0f0f22", padding:"10px 10px", gap:8, borderBottom:"2px solid #7c4dff22" }}>
      {CATEGORY_TABS.map((tab, i) => {
        ...
        return (
          ...
          >
            <div style={{ width:82, height:58, borderRadius:10, position:"relative", overflow:"hidden", border: lit ? `2px solid ${tab.glow}` : "2px solid rgba(255,255,255,0.07)", boxShadow: active ? `0 0 22px ${tab.glow}99, 0 4px 16px rgba(0,0,0,0.6)` : hovered ? `0 0 14px ${tab.glow}55, 0 4px 12px rgba(0,0,0,0.5)` : "0 2px 8px rgba(0,0,0,0.5)", transform: lit ? "translateY(-4px)" : "none", transition:"all 0.3s cubic-bezier(.2,.8,.3,1)" }}>
              <div style={{ position:"absolute", inset:0, background: tab.bg }}/>
              <div style={{ position:"absolute", inset:0, transform: lit ? "scale(1.08)" : "scale(1)", transition:"transform 0.4s ease", display:"flex", alignItems:"center", justifyContent:"center" }}>{tab.scene}</div>
              ...
              <div style={{ position:"absolute", bottom:4, left:0, right:0, textAlign:"center", zIndex:4, fontSize:8, fontWeight:900, color:"#fff", letterSpacing:0.5, textTransform:"uppercase", textShadow:"0 1px 4px rgba(0,0,0,0.9)", fontFamily:"'Oswald',sans-serif" }}>{tab.label}</div>
              <div style={{ position:"absolute", top:0, left:0, right:0, height:2, zIndex:4, background: active ? `linear-gradient(90deg,transparent,${tab.glow},transparent)` : "transparent", transition:"background 0.3s" }}/>
              <div style={{ position:"absolute", inset:0, zIndex:2, background: lit ? `radial-gradient(circle at 50% 40%,${tab.glow}25,transparent 70%)` : "transparent", transition:"background 0.3s" }}/>
            </div>
            <span style={{ fontSize:9, fontWeight:700, letterSpacing:0.3, color: active ? tab.glow : "#555", whiteSpace:"nowrap", transition:"color 0.2s", textTransform:"uppercase" }}>{tab.label}</span>
          </button>
        );
      })}
    </div>
...
function addRipple(e) {
  ...
  Object.assign(circle.style, { position:"absolute", width:`${d}px`, height:`${d}px`, borderRadius:"50%", background:"rgba(255,255,255,0.25)", pointerEvents:"none", zIndex:10, top:`${e.clientY - rect.top - d/2}px`, left:`${e.clientX - rect.left - d/2}px`, animation:"ripple 0.6s ease-out forwards" });
...
function OddsCell({ value, volume }) {
  ...
  if (value === "-") {
    return (
      <div style={{ display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", background:"#0f0f28", borderRadius:6, padding:"6px 4px", minHeight:44, border:"1px solid #ffffff08" }}>
        <span style={{ color:"#333", fontSize:16, lineHeight:1 }}>—</span>
      </div>
    );
  }
  return (
    <button className="odds-btn" onClick={addRipple} style={{ background: flash ? "linear-gradient(135deg,#ff6d00,#ffd740)" : "linear-gradient(135deg,#1e1560,#2d1f80)", border: flash ? "1px solid #ffd740" : "1px solid #7c4dff55", borderRadius:6, padding:"5px 4px", cursor:"pointer", color: flash ? "#000" : "#fff", fontWeight:800, fontSize:15, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:2, transition:"background 0.3s, transform 0.2s, filter 0.2s", width:"100%", minHeight:44, position:"relative", overflow:"hidden" }}>
      <span style={{ lineHeight:1, letterSpacing:0.5 }}>{value}</span>
      {volume && <span style={{ fontSize:8, opacity: flash ? 0.7 : 0.55, fontWeight:600, color: flash ? "#000" : "#a990ff", letterSpacing:0.3 }}>{volume}</span>}
    </button>
  );
}
...
function MatchCard({ match, index, visible }) {
  ...
  return (
    <div className="match-card" onClick={() => openWhatsApp(`Hello, I want to bet on ${match.t1} vs ${match.t2} on MangoPlay!`)} style={{ borderRadius:12, marginBottom:10, overflow:"hidden", border:"1px solid #ffffff12", transition:"all 0.25s", cursor:"pointer", animation: visible ? `cardEntrance 0.5s ease ${index * 0.07}s both` : "none", opacity: visible ? 1 : 0, boxShadow:"0 4px 20px rgba(0,0,0,0.5)" }}>
      <div style={{ background:"linear-gradient(90deg,#0f0f22,#1a1a38)", padding:"5px 10px", display:"flex", alignItems:"center", justifyContent:"space-between", borderBottom:"1px solid #ffffff08" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr auto 1fr" }}>
        <div style={{ background: c1.bg, padding:"10px 12px", display:"flex", flexDirection:"column", justifyContent:"center", gap:3, position:"relative", overflow:"hidden" }}>
          <div style={{ position:"absolute", top:0, right:0, bottom:0, width:"40%", background:"linear-gradient(90deg,transparent,rgba(0,0,0,0.3))", pointerEvents:"none" }} />
          <div style={{ display:"inline-block", background: c1.badge, borderRadius:3, padding:"1px 6px", fontSize:8, fontWeight:800, color: c1.accent === c1.badge ? "#fff" : c1.accent, letterSpacing:0.5, width:"fit-content", marginBottom:2 }}>HOME</div>
          <div style={{ fontSize:12, fontWeight:800, color:"#fff", lineHeight:1.2, textShadow:"0 1px 4px rgba(0,0,0,0.6)" }}>{match.t1}</div>
        </div>
        <div style={{ background:"#0a0a1e", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"0 10px", gap:2, borderLeft:"1px solid #ffffff08", borderRight:"1px solid #ffffff08" }}>
          <div style={{ fontSize:9, fontWeight:900, color:"#444", letterSpacing:2 }}>VS</div>
        </div>
        <div style={{ background: c2.bg, padding:"10px 12px", display:"flex", flexDirection:"column", justifyContent:"center", alignItems:"flex-end", gap:3, position:"relative", overflow:"hidden" }}>
          <div style={{ position:"absolute", top:0, left:0, bottom:0, width:"40%", background:"linear-gradient(270deg,transparent,rgba(0,0,0,0.3))", pointerEvents:"none" }} />
          <div style={{ display:"inline-block", background: c2.badge, borderRadius:3, padding:"1px 6px", fontSize:8, fontWeight:800, color: c2.accent === c2.badge ? "#fff" : c2.accent, letterSpacing:0.5, width:"fit-content", marginBottom:2 }}>AWAY</div>
          <div style={{ fontSize:12, fontWeight:800, color:"#fff", lineHeight:1.2, textAlign:"right", textShadow:"0 1px 4px rgba(0,0,0,0.6)" }}>{match.t2}</div>
        </div>
      </div>
      <div style={{ background:"#0d0d22", padding:"3px 12px", fontSize:9, color:"#444", display:"flex", alignItems:"center", gap:3, borderTop:"1px solid #ffffff06" }}>📍 {match.venue}</div>
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", padding:"6px 10px 0", gap:6, background:"#0d0d22" }}>
        {["1", "X", "2"].map(label => (<div key={label} style={{ textAlign:"center", fontSize:9, color:"#7c4dff", fontWeight:800, letterSpacing:1, background:"#7c4dff18", borderRadius:"4px 4px 0 0", padding:"2px 0" }}>{label}</div>))}
      </div>
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", padding:"0 10px 10px", gap:6, background:"#0d0d22" }}>
...
function MangoWelcomeBanner({ animKey }) {
  return (
    <div style={{ position:"relative", overflow:"hidden", minHeight:180, background:"linear-gradient(135deg,#0a0015 0%,#1e0040 35%,#0d1040 70%,#050a20 100%)" }}>
      {[...Array(16)].map((_,i) => (<div key={i} style={{ position:"absolute", width: i%3===0 ? 3 : 2, height: i%3===0 ? 3 : 2, borderRadius:"50%", background: i%4===0 ? "#f90" : "#ffffff", top:`${3 + i*6}%`, left:`${2 + i*7}%`, opacity: 0.2 + (i%5)*0.1, animation:`particleFloat ${2 + i*0.3}s ease-in-out infinite`, animationDelay:`${i*0.15}s`, pointerEvents:"none", boxShadow: i%4===0 ? "0 0 4px #f90" : "0 0 3px #fff" }} />))}
...
function HeroBanner() {
    ...
    return (
      <div style={{ position:"relative" }}>
        ...
        <div style={{ position:"absolute", bottom:8, left:"50%", transform:"translateX(-50%)", display:"flex", gap:6, zIndex:3 }}>
          {BANNERS.map((_,i) => (<div key={i} onClick={() => { setCurrent(i); setAnimKey(k=>k+1); }} style={{ width:i===current?20:6, height:6, borderRadius:3, background:i===current?"#f90":"#ffffff33", transition:"all 0.4s", cursor:"pointer" }} />))}
        </div>
      </div>
    );
  }
  return (
    <div style={{ position:"relative", overflow:"hidden", minHeight:175 }}>
      ...
      {particles.map(i => (<div key={i} style={{ position:"absolute", fontSize:12+i*3, opacity:0.1+i*0.02, top:`${8+i*11}%`, left:`${4+i*13}%`, animation:`particleFloat ${2.4+i*0.5}s ease-in-out infinite`, animationDelay:`${i*0.25}s`, pointerEvents:"none" }}>{b.particle}</div>))}
      ...
      <div style={{ position:"relative", zIndex:2, padding:"22px 18px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
        <div style={{ flex:1 }}>
          ...
          <div key={`d-${animKey}`} style={{ marginTop:12, background:`linear-gradient(90deg,${b.accent}cc,${b.accent}44)`, borderRadius:20, padding:"5px 14px", display:"inline-block", fontSize:11, fontWeight:700, color:"#fff", border:`1px solid ${b.accent}66`, animation:"pulse 2s infinite, fadeUp 0.9s ease both" }}>{b.bonus}</div>
        </div>
        <div key={`e-${animKey}`} style={{ fontSize:70, lineHeight:1, animation:"slideInRight 0.6s ease both, float 3s ease-in-out infinite", filter:`drop-shadow(0 0 24px ${b.accent}99)`, flexShrink:0, marginLeft:12 }}>{b.emoji}</div>
      </div>
      <div style={{ position:"absolute", bottom:8, left:"50%", transform:"translateX(-50%)", display:"flex", gap:6, zIndex:3 }}>
        {BANNERS.map((_,i) => (<div key={i} onClick={() => { setCurrent(i); setAnimKey(k=>k+1); }} style={{ width:i===current?20:6, height:6, borderRadius:3, background:i===current?b.accent:"#ffffff33", transition:"all 0.4s", cursor:"pointer" }} />))}
      </div>
    </div>
  );
}
    ...
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.88)", zIndex:300, display:"flex", alignItems:"center", justifyContent:"center", backdropFilter:"blur(5px)" }}>
      <div style={{ background:"linear-gradient(160deg,#16162e,#0d0d20)", borderRadius:18, padding:28, width:320, border:"2px solid #7c4dff", animation:"slideDown 0.3s ease", maxHeight:"90vh", overflowY:"auto" }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20 }}>
          ...
          <span onClick={onClose} style={{ cursor:"pointer", fontSize:20, color:"#888", transition:"color 0.2s" }} onMouseEnter={e=>e.target.style.color="#fff"} onMouseLeave={e=>e.target.style.color="#888"}>✕</span>
        </div>
        {children}
      </div>
    </div>
  ...
  return (
    <div style={{ background:"linear-gradient(160deg,#0d0d22,#120a28)", padding:"24px 14px 20px", position:"relative", overflow:"hidden" }}>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
        {steps.map((s, i) => (
          <div key={i} style={{ display:"flex", alignItems:"center", gap:14, background:"linear-gradient(135deg,#13132b,#1a1535)", border:`1px solid ${s.color}33`, borderRadius:14, padding:"14px 16px", position:"relative", overflow:"hidden", transition:"all 0.2s" }}
      ...
      <div style={{ textAlign:"center", marginTop:18 }}>
        <button onClick={()=>openWhatsApp("Hello, I want to create my ID on MangoPlay!")} style={{ background:"linear-gradient(135deg,#25D366,#128C7E)", border:"none", color:"#fff", padding:"12px 28px", borderRadius:40, fontWeight:800, fontSize:13, cursor:"pointer", fontFamily:"'Oswald',sans-serif", letterSpacing:1, boxShadow:"0 4px 20px rgba(37,211,102,0.4)", display:"inline-flex", alignItems:"center", gap:8 }}>
...
function TrustFooter() {
  ...
  return (
    <div style={{ position:"relative", overflow:"hidden", paddingBottom:90 }}>
      ...
      <div style={{ background:"linear-gradient(135deg,#0f0f28 0%,#1a0a35 50%,#0f0f28 100%)", display:"grid", gridTemplateColumns:"1fr 1fr 1fr", borderBottom:"1px solid #7c4dff22" }}>
        {stats.map((s,i) => (
          <div key={i} style={{ textAlign:"center", padding:"16px 8px 12px", borderRight:i<2?"1px solid #7c4dff18":"none", position:"relative", overflow:"hidden" }}>
      ...
      <div style={{ background:"linear-gradient(160deg,#0d0d22 0%,#150a30 40%,#0a1220 100%)", padding:"28px 18px 24px", textAlign:"center", position:"relative", overflow:"hidden" }}>
        ...
        {[...Array(10)].map((_,i) => (<div key={i} style={{ position:"absolute", width:i%3===0?3:2, height:i%3===0?3:2, borderRadius:"50%", background:i%2===0?"#7c4dff":"#f90", top:`${5+i*9}%`, left:`${3+i*10}%`, opacity:0.15, animation:`particleFloat ${2.5+i*0.4}s ease-in-out infinite`, animationDelay:`${i*0.2}s`, pointerEvents:"none" }} />))}
        <div style={{ position:"relative", zIndex:2 }}>
          <div style={{ display:"inline-block", background:"linear-gradient(90deg,#7c4dff22,#f9900022)", border:"1px solid #7c4dff44", borderRadius:20, padding:"4px 16px", fontSize:10, fontWeight:700, color:"#7c4dff", letterSpacing:2, marginBottom:12 }}>🥭 INDIA'S TRUSTED ONLINE BOOK</div>
          ...
          <div style={{ fontSize:11, color:"#888", marginBottom:22, letterSpacing:0.5 }}>Over <span style={{ color:"#f90", fontWeight:700 }}>50,000 players</span> trust us every day</div>
          <div style={{ position:"relative", display:"inline-block", marginBottom:20 }}>
            ...
            <a href="#" onClick={e=>{e.preventDefault();openWhatsApp("Hello, I want to create my ID on MangoPlay!");}}
              style={{ position:"relative", display:"inline-flex", alignItems:"center", gap:10, background:"linear-gradient(135deg,#25D366 0%,#1ebe5d 50%,#128C7E 100%)", color:"#fff", padding:"14px 32px", borderRadius:50, fontWeight:900, fontSize:15, textDecoration:"none", boxShadow:"0 6px 30px rgba(37,211,102,0.5)", border:"1px solid rgba(255,255,255,0.2)", fontFamily:"'Oswald',sans-serif", letterSpacing:1, overflow:"hidden" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, marginBottom:20 }}>
            {features.map((f,i) => (
              <div key={i} style={{ background:"linear-gradient(135deg,#12122a,#1a1535)", border:"1px solid #7c4dff22", borderRadius:12, padding:"12px 10px", textAlign:"left", display:"flex", alignItems:"center", gap:10, transition:"all 0.25s" }}
          ...
          <div style={{ marginBottom:20 }}>
            ...
            <div style={{ display:"flex", justifyContent:"center", gap:8, flexWrap:"wrap" }}>
              {payments.map((p) => (<div key={p.label} style={{ background:"linear-gradient(135deg,#12122a,#1c1540)", padding:"8px 14px", borderRadius:10, fontSize:11, fontWeight:700, color:"#bbb", border:"1px solid #7c4dff33", display:"flex", alignItems:"center", gap:6, boxShadow:"0 2px 10px rgba(0,0,0,0.3)" }}><span style={{ fontSize:16 }}>{p.icon}</span>{p.label}</div>))}
            </div>
          </div>
          <div style={{ height:1, background:"linear-gradient(90deg,transparent,#7c4dff44,#f90044,#7c4dff44,transparent)", marginBottom:14 }} />
          <div style={{ fontSize:9, color:"#333", marginBottom:10 }}>Mango Online Book — Trusted by 50,000+ players across India</div>
          <div style={{ display:"inline-flex", alignItems:"center", gap:6, background:"linear-gradient(135deg,#1a0a0a,#2a0a0a)", border:"1px solid #ff444433", borderRadius:8, padding:"5px 14px", fontSize:9, color:"#ff6666", fontWeight:700, letterSpacing:1 }}>
...
function GameCard({ card, index }) {
  ...
  return (
    ...
    >
      ...
      {[0,1,2,3].map(j=>(<div key={j} style={{ position:"absolute", width: j%2===0?4:3, height: j%2===0?4:3, borderRadius:"50%", background:card.glow, top:`${12+j*19}%`, left:`${8+j*23}%`, opacity: hovered ? 0.8 : 0.2, boxShadow:`0 0 ${hovered?10:4}px ${card.glow}`, animation:`orbFloat ${2.2+j*0.6}s ease-in-out infinite`, animationDelay:`${j*0.35}s`, transition:"opacity 0.3s, box-shadow 0.3s", zIndex:3, pointerEvents:"none" }}/>))}
      ...
      <div style={{ position:"absolute", top:0, bottom:0, width:"55%", background:"linear-gradient(90deg,transparent,rgba(255,255,255,0.08),transparent)", left: hovered ? "120%" : "-70%", transition:"left 0.5s ease", zIndex:5, pointerEvents:"none" }}/>
      ...
      <div style={{ position:"absolute", inset:0, borderRadius:16, zIndex:7, pointerEvents:"none", border: hovered ? `1.5px solid ${card.glow}` : "1.5px solid rgba(255,255,255,0.06)", boxShadow: hovered ? `inset 0 0 20px ${card.glow}22` : "none", transition:"border 0.3s ease, box-shadow 0.3s ease" }}/>
      <div style={{ position:"absolute", top:9, left:9, zIndex:9, background:`linear-gradient(135deg,${card.glow},${card.glow}88)`, borderRadius:7, padding:"2px 9px", fontSize:8, fontWeight:900, color:"#000", letterSpacing:0.8, boxShadow:`0 2px 8px ${card.glow}66` }}>{card.badge}</div>
      ...
      <div style={{ position:"absolute", zIndex:9, bottom: hovered ? 33 : 10, left:10, right:10, transition:"bottom 0.3s cubic-bezier(.2,.8,.3,1)" }}>
        ...
        <div style={{ padding:"10px 12px 6px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:8 }}>
            ...
            <span style={{ fontSize:11, color:"#7c4dff", cursor:"pointer", fontWeight:700, transition:"color 0.2s" }}
              ...
              onMouseLeave={e=>e.target.style.color="#7c4dff"}>VIEW ALL →</span>
          </div>
        </div>
